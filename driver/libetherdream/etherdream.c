...
static void *watch_for_dacs(void *arg) {
  ...
  while (1) {
    ...
    /* See if config is active and this DAC is concerned */
    if( config ){
      char conf_addr[16];
      strcpy( conf_addr, config);
      char sock_addr[16];
      inet_ntop(AF_INET, &(src.sin_addr.s_addr), sock_addr, INET_ADDRSTRLEN);
      if( strcmp(conf_addr, sock_addr) != 0 ){
        continue;
      }
    }
...
int etherdream_lib_start(void) {
  ...
  config = getenv ("IP_ADDRESS" );
  if( config ){
    trace(NULL, "== IP_ADDRESS received %s ==\n", config);
  }
